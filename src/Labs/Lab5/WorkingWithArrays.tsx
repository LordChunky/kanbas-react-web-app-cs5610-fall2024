...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
            <input className="form-check-input" type="checkbox" value="true" id="wd-todo-completion-status1"
                onChange={(e) => setTodo({ ...todo, completed: (e.target.value === "true") })}/> 
            <label className="form-check-label ms-1" htmlFor="wd-todo-completion-status1">
                True
            </label>

            <input className="form-check-input ms-2" type="checkbox" value="false" id="wd-todo-completion-status2"
                onChange={(e) => setTodo({ ...todo, completed: (e.target.value === "true") })}/> 
            <label className="form-check-label ms-1" htmlFor="wd-todo-completion-status2">
                False
            </label> <br/> <br/> <br />
            {/* <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
            ...
            <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
            <textarea className="form-control w-50 float-start me-2 " id="wd-module-description"
